...
class LoginState {
  LoginState({this.loginStateType = LoginStateType.waiting, this.message, this.error, this.stackTrace});

  final LoginStateType loginStateType;
...
class LoginBloc extends BlocBase {
  ...
  factory LoginBloc(MyLocalization myLocalization, LoginRepository loginRepository){
    if (_bloc._loginRepository == null){
      ...
      _bloc._loginStateController.add(LoginState(loginStateType: LoginStateType.waiting));
  ...
  void onLoginTapped() {
    _loginStateController.add(LoginState(loginStateType: LoginStateType.loading));
    _loginRepository.login(username: _usernameController.value, password: _passwordController.value)
      .then((String securityToken) {
        ...
        _loginStateController.add(LoginState(loginStateType: LoginStateType.finished));
    }).catchError((dynamic error, StackTrace stackTrace) =>
      _loginStateController.add(LoginState(loginStateType: LoginStateType.error, error: error, stackTrace: stackTrace)));
  }
  ...
  Future<void> onGoogleLoginTapped() async {
    try {
      ...
      if (googleSignInAuthentication?.idToken != null){
        ...
        _loginStateController.add(LoginState(loginStateType: LoginStateType.error, message: _myLocalization.facebookLoginFailed));
      }
    } catch (error) {
      print(error);
      _loginStateController.add(LoginState(loginStateType: LoginStateType.error, message: _myLocalization.googleLoginFailed));
    }
  }
