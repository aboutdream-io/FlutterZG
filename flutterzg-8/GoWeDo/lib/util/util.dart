...
class Util {
  ...
  static ImageProvider getImageProvider(String url, {String placeholder, bool disableMemoryCache = false}) {
    return url?.isNotEmpty == true ? AdvancedNetworkImage(url, disableMemoryCache: disableMemoryCache, useDiskCache: true,
        fallbackAssetImage: placeholder, printError: true) : AssetImage(placeholder);
  }

  static Widget getImage(String url,
      {bool withWrapper = false, bool disableMemoryCache = false, BoxFit fit = BoxFit.contain, String placeholder}){
    final ImageProvider imageProvider = getImageProvider(url, disableMemoryCache: disableMemoryCache, placeholder: placeholder);
    return withWrapper ? TransitionToImage(image: imageProvider, fit: fit) : Container(
