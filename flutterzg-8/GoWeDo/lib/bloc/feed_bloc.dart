...
class FeedBloc extends BlocBase {
  ...
  factory FeedBloc(MyLocalization myLocalization, PostRepository postRepository){
    if (_bloc._repository == null){
      ...
      _bloc._notificationsController = BehaviorSubject<List<NewPostNotification>>.seeded([]);
    }
    return _bloc;
  }
  FeedBloc._internal();
  ...
  BehaviorSubject<List<NewPostNotification>> _notificationsController;
  ...
  void refreshFeed() {
    ...
    _repository.getPosts(offset: 0, limit: 10)
      .then((posts) {
        _stateController.add(FeedState(
            stateType: StateType.waiting,
            posts: posts,
